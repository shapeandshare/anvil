...
def upgrade():
    ...
    op.add_column(
        "datasets",
        sa.Column("curation_version", sa.Integer(), server_default="0", nullable=False),
    )
    op.add_column(
        "datasets",
        sa.Column("status", sa.String(20), server_default="empty", nullable=False),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "is_removed", sa.Boolean(), server_default="0", nullable=False, index=True
        ),
        sa.Column(
            "removed_by_op_id",
            sa.Integer(),
            sa.ForeignKey("curation_operations.id"),
            nullable=True,
        ),
        sa.Column(
            "import_source_id",
            sa.Integer(),
            sa.ForeignKey("import_sources.id"),
            nullable=True,
        ),
        sa.Column(
            "created_at", sa.DateTime(), server_default=sa.func.now(), nullable=False
