class Tokenizer:
    ...
    def encode(self, doc: str) -> list[int]:
        return [self.BOS] + [self.uchars.index(ch) for ch in doc if ch in self.uchars] + [self.BOS]
...
class Vocabulary:
    ...
    def encode(self, text: str) -> list[int]:
        return [self.bos_id] + [self._char_to_id[ch] for ch in text if ch in self._char_to_id] + [self.bos_id]
