...
- **`env.paused`** (passed into the factory) gates all motion; the system separately handles reduced-motion / legibility by pausing the loop and zeroing canvas opacity via `onEffectLevelChange`, so an effect does not need its own reduced-motion branch — it only needs to honour `env.paused` in `update`.
