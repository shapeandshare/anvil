---
title: 007 Learning Content Enrichment - tasks
type: tasks
tags:
  - type/spec
spec-refs:
...
**Input**: Design documents from `docs/vault/Specs/007 Learning Content Enrichment/`
...
- Python package: `anvil/`
- Widget JS: `anvil/api/static/js/widgets/`
- Widget HTML: `anvil/api/templates/partials/concept-widgets/`
- Lesson templates: `anvil/api/templates/archetypes/`
- CSS: `anvil/api/static/css/`
- Examples: `examples/`
- Tests: `tests/`
...
- [X] T001 Review all design artifacts in `docs/vault/Specs/007 Learning Content Enrichment/` — plan.md, spec.md, research.md, data-model.md, 4 contract files, quickstart.md
...
- [X] T004 Add `backward_graph()` method to InferenceService in `anvil/services/inference.py` — runs forward pass on input text, computes loss, calls `loss.backward()`, then traverses Value graph capturing `.data`, `.grad`, `.local_grads` per node. Capped at 400 nodes.
- [X] T005 [P] Add `loss_breakdown()` method to InferenceService in `anvil/services/inference.py` — tokenizes input, runs forward pass per position, computes per-token cross-entropy + running average + random-guess baseline (`-log(1/vocab_size)`)
- [X] T006 [P] Add `model_params()` method to InferenceService in `anvil/services/inference.py` — iterates `GPT.state_dict`, extracts shape/param count per named matrix, categorizes into embedding/attention/mlp/output groups
- [X] T007 Add routes for new endpoints in `anvil/api/v1/inference.py` — `POST /v1/inference/backward-graph`, `POST /v1/inference/loss-breakdown`, `GET /v1/inference/model-params`
...
- [X] T011 Create `AutogradWidget` in `anvil/api/static/js/widgets/autograd.js` — canvas-based computation graph renderer. Constructor takes container. `_fetch(text)` calls `POST /v1/inference/backward-graph`. `_renderOutput(data)` renders nodes as rounded rectangles with `.data` value, `.grad` gradient, and operation type label. Color-coded by op type (input=green, add=blue, mul=orange, pow=purple, log=amber, exp=red, relu=cyan). Edges drawn as arrows from parent to child with `_local_grads` annotated inline.
- [X] T012 Create Autograd widget HTML partial in `anvil/api/templates/partials/concept-widgets/autograd.html` — canvas element with text input and hint text
- [X] T013 [P] Register `AutogradWidget` in WIDGET_CLASSES and add script tag in `anvil/api/templates/archetypes/concept.html`
- [X] T014 Define `AUTOGRAD_STEPS` (5 steps) in `anvil/api/v1/router.py`:
...
- [X] T015 Add route handler `GET /v1/learn/autograd` and entry to `LEARNING_ARC` in `anvil/api/v1/router.py`
- [X] T016 Add Autograd widget CSS to `anvil/api/static/css/components.css` — `.autograph-canvas` sizing, `.autograd-legend` for op type colors, `.grad-value` styling
...
- [X] T018 Implement `examples/train3.py` — Single-head causal self-attention with learned position embeddings, RMSNorm, residual connections, single transformer block. Train with SGD. Print loss per step. Reuse `linear()`, `softmax()`, `rmsnorm()` from `anvil/core/engine.py`. Stdlib only.
- [X] T019 Implement `examples/train4.py` — Multi-head GPT (n_head divisible into n_embd) with learned embeddings, RMSNorm, multi-head attention, residual MLP, single transformer layer. Train with SGD. Print loss per step. Reuse `GPT.forward()` from `anvil/core/engine.py`. Stdlib only.
...
- [X] T021 Create `LossWidget` in `anvil/api/static/js/widgets/loss.js` — bar chart rendering per-token loss as vertical bars. Highlights tokens with high loss (red) vs low loss (green). Shows average loss as dashed horizontal line. Shows random-guess baseline as dotted line with annotation. `_fetch(text)` calls `POST /v1/inference/loss-breakdown`.
- [X] T022 Create Loss widget HTML partial in `anvil/api/templates/partials/concept-widgets/loss.html` — text input + canvas for bar chart + statistics display
- [X] T023 [P] Register `LossWidget` in WIDGET_CLASSES and add script tag in `anvil/api/templates/archetypes/concept.html`
- [X] T024 Define `LOSS_STEPS` (5 steps) in `anvil/api/v1/router.py`:
...
- [X] T025 Add route handler `GET /v1/learn/loss` and entry to `LEARNING_ARC` in `anvil/api/v1/router.py`
- [X] T026 Add Loss widget CSS to `anvil/api/static/css/components.css` — `.loss-bar` colors, `.baseline-line` styling
...
- [X] T027 Create `ParamsWidget` in `anvil/api/static/js/widgets/params.js` — renders parameter breakdown as stacked horizontal bar or treemap. Color-coded by category (embedding=blue, attention=cyan, mlp=orange, output=purple). Shows shape, count, percentage per group. Includes `n_embd` and `n_layer` sliders that call `GET /v1/inference/model-params` with updated values and re-render.
- [X] T028 Create Params widget HTML partial in `anvil/api/templates/partials/concept-widgets/params.html` — canvas for visualization + slider controls + stats table
- [X] T029 [P] Register `ParamsWidget` in WIDGET_CLASSES and add script tag in `anvil/api/templates/archetypes/concept.html`
- [X] T030 Define `PARAMS_STEPS` (5 steps) in `anvil/api/v1/router.py`:
...
- [X] T031 Add route handler `GET /v1/learn/parameters` and entry to `LEARNING_ARC` in `anvil/api/v1/router.py`
- [X] T032 Add Params widget CSS to `anvil/api/static/css/components.css`
...
- [X] T033 Add `optimizer_state_callback` parameter to `train()` in `anvil/core/engine.py` — called after Adam update, passes `(step, m, v, grads)` arrays. Optional parameter, defaults to None. Captures optimizer state at configurable interval (default: every 10 steps).
- [X] T034 Extend `TrainingService` in `anvil/services/training.py` — wire `optimizer_state_callback` to emit new `optimizer_state` SSE event type with `{"step": N, "params": [{"index": i, "m": m[i], "v": v[i], "grad": p.grad}]}` payload. Only for CPU training path; no optimizer state events for GPU path.
- [X] T035 Create `AdamWidget` in `anvil/api/static/js/widgets/adam.js` — dual line chart showing m (momentum) and v (adaptive LR) curves over training steps for a selected parameter. Includes beta1/beta2 sliders that show how momentum and adaptive LR response curves change. Shows LR decay annotation `lr_t = lr * (1 - step/num_steps)`. Listens for `optimizer_state` SSE events when training is active, or loads from stored experiment data.
- [X] T036 Create Adam widget HTML partial in `anvil/api/templates/partials/concept-widgets/adam.html` — canvas for dual chart + beta1/beta2 sliders + parameter selector dropdown
- [X] T037 [P] Register `AdamWidget` in WIDGET_CLASSES and add script tag in `anvil/api/templates/archetypes/concept.html`
- [X] T038 Define `ADAM_STEPS` (5 steps) in `anvil/api/v1/router.py`:
...
- [X] T039 Add route handler `GET /v1/learn/adam` and entry to `LEARNING_ARC` in `anvil/api/v1/router.py`
- [X] T040 Add Adam widget CSS to `anvil/api/static/css/components.css`
...
- [X] T041 Create FAQ page template in `anvil/api/templates/archetypes/faq.html` — extends base.html. Accordion-style layout with question headers and expandable answer sections. Styled consistently with existing panels.
- [X] T042 Add route handler `GET /v1/learn/faq` and entry to `LEARNING_ARC` in `anvil/api/v1/router.py`
- [X] T043 Add FAQ CSS to `anvil/api/static/css/archetypes.css` — `.faq-item`, `.faq-question`, `.faq-answer`, `.faq-answer--open` accordion styles
...
- [X] T044 Extend `ATTENTION_STEPS` array in `anvil/api/v1/router.py` — add 2 new steps after existing step 5:
...
- [X] T045 [P] Update the attention widget in `anvil/api/static/js/widgets/attention.js` — add optional rendering mode for residual stream visualization when step key matches "residual". Uses existing per-token embedding data from the attention endpoint (`POST /v1/inference/attention`) to illustrate the `x = [a + b for a, b in zip(x, x_residual)]` pattern. RMSNorm step uses a schematic diagram showing: input vector → RMS computation (`sqrt(mean(x²))`) → scale factor → normalized output. No new backend data needed.
...
- [X] T052 Update `anvil/api/v1/router.py` LEARNING_ARC ordering — ensure new lessons appear in logical pedagogical order: Tokenization → Embeddings → Parameters → Autograd → Attention → Loss → Sampling → Adam → Training Loop → FAQ
