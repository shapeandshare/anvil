...
- **FR-008**: A local orchestration definition (docker compose) MUST bring the installed anvil online locally as a single app service (with MLflow running in-process inside that container, consistent with current behavior), exposing the web port (8080) ~~and the in-process MLflow port (5001)~~ and provisioning required configuration (ports, runtime workspace).
  > **Superseded in part (2026-06-28)**: The clause publishing MLflow port `5001` is SUPERSEDED by [[Specs/024 Unified Interface Local TLS/024 Unified Interface Local TLS|Spec 024]] (FR-002) and [[Specs/056 Reverse-Proxy Registry/056 Reverse-Proxy Registry|Spec 056]] (FR-002/FR-009), which require MLflow to bind loopback-only with its host port UNPUBLISHED (reachable solely via the authenticated `/v1/mlflow-proxy/`). When 056 lands, `compose.yaml` MUST publish only `8080:8080` and `Dockerfile` MUST `EXPOSE 8080` only. The single-origin proxy architecture (ADR-037) is the governing decision for this port; this spec defers to it.
