...
10. **Forward References via PEP 563** — Never use string-literal forward references (`"MyClass"`) in type annotations. Instead, add `from __future__ import annotations` (PEP 563) at the top of the file, which defers all annotation evaluation to strings automatically. For cross-module type references, default to a normal top-level import (the project's layered architecture should resolve cross-module dependencies without cycles). Use `TYPE_CHECKING`-guarded imports ONLY to break a genuine runtime circular import that cannot be resolved without violating another rule. See Constitution Additional Constraints for the full exception discipline, including the four conditions (annotations marker, genuine cycle, annotation-only usage, one-line cycle comment). A script (`scripts/ci/check_guarded_imports.py`) enforces that guarded symbols stay annotation-only.

   **ORM model cycles**: bidirectional SQLAlchemy `relationship()` declarations between co-dependent models are the most common source of genuine cycles. These SHOULD be resolved by **merging the mutually-dependent classes into a single file** (the ``__init__.py`` Ownership Policy permits multi-class files for this purpose). This eliminates the need for `TYPE_CHECKING`-guarded imports entirely in the model layer. The remaining 15+ non-model uses of `TYPE_CHECKING` in the codebase are for annotation-only forward references to types from optional dependencies or genuinely unavoidable cycles in the service layer.

   Permitted (genuine unavoidable cycle, non-ORM):
   ```python
...
- Python 3.11+ (existing repo convention, PEP 604, `StrEnum`, `from __future__ import annotations`) + FastAPI, async SQLAlchemy + aiosqlite, Jinja2 (existing); `peft`, `bitsandbytes` (behind `[finetune]` extra, from 044); `huggingface_hub` (behind `[finetune]` extra, from 040); `boto3` / `aioboto3` (behind `[saas]` extra, from 032); LakeFS client (from 019) (047-saas-fine-tuning-pipeline)
