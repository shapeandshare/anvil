...
async def test_lifecycle_journey(client: Any) -> None:
    ...
    from tests.e2e.api.conftest import TINY_TRAINING_CONFIG
    from tests.e2e.api.conftest import make_corpus
    from tests.e2e.api.conftest import make_dataset
    from tests.e2e.api.conftest import make_eval_dataset
    from tests.e2e.api.conftest import poll_until_terminal
    ...
    try:
        ...
        corpus_id = corpus["id"]
        assert isinstance(corpus_id, (int, str)), (
        ...
        assert isinstance(dataset_id, (int, str)), (
        ...
        if experiment_id is not None:
            ...
            assert len(loss_series) > 0, (
                "Step 5b (exp metrics): loss series is empty"
            )
