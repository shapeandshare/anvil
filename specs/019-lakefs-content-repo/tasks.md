...
**Input**: Design documents from `/specs/019-lakefs-content-repo/`
...
> **The SaaS requirements are now captured in `specs/016-saas-architecture/spec.md`
...
- [X] T095 [P] Write `docs/vault/Decisions/ADR-033-content-repository-substrate.md` recording the LakeFS-vs-pure-Python-local decision + `VersionedContentStore` boundary; include a **"SaaS integration hand-off"** section cross-linking `specs/016-saas-architecture` (FR-062–067, SC-021) and ADR-030 AD-17 (LakeFS SaaS substrate, app-level RBAC, in-process validation, manifest-digest parity). Add ADR-033 to the `docs/vault/Decisions/README.md` index. (Constitution + plan require an ADR.)
