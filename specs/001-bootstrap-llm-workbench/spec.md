...
3. **Phase 3 — Remainder**: Core training (`microgpt.py`, progressive files), FastAPI web server, MLflow integration, operations dashboard, dataset management, experiment tracking UI, GPU support, Alembic migrations, all CRUD endpoints, versioned API.
...
- **FR-001**: Repository MUST include Karpathy's `microgpt.py` as the core training and inference script, with zero third-party Python dependencies (stdlib only).
...
- **FR-022**: The core `microgpt.py` script MUST remain zero-dependency (stdlib only); all web UI code, experiment tracking via MLflow, and GPU acceleration MUST be opt-in layers that import their respective dependencies only when activated.
...
- **FR-029**: Repository MUST include a `pyproject.toml` with package metadata (`name="microgpt-workbench"`, version using semantic versioning, dependencies, optional dependency groups, entry points) so the entire system is installable via `pip install .` or `pip install -e .`. The `microgpt` core training module MUST be importable as a namespace package after installation. Repository MUST include a dependency lock file (e.g., `uv.lock`, `requirements.lock`) that pins all transitive dependency versions for reproducible installs. The `make setup` target MUST install from the lock file.
...
- **FR-033**: Semantic versioning (MAJOR.MINOR.PATCH) MUST be used for all releases. The current version MUST be declared in `pyproject.toml` and accessible via `microgpt.__version__`. Version bumps MUST follow conventional commit analysis.
...
- **FR-048**: The `pyproject.toml` MUST define CLI entry points under `[project.scripts]` for all major operations: `microgpt-workbench` (starts the web server), `microgpt-train` (runs training from CLI), `microgpt-stop` (stops services). These entry points delegate to the god class.
...
- **pyproject.toml**: Python package configuration declaring the `microgpt-workbench` package with semver, dependencies, optional extras, flake8/ruff/black/isort config, mypy/pyright config, and entry points. Enables `pip install .` distribution.
...
- **SC-011**: The core `microgpt.py` script imports zero third-party packages — running `python microgpt.py` with only stdlib installed works without errors.
...
- **SC-024**: The package version follows semantic versioning in `pyproject.toml`; `microgpt.__version__` is accessible at runtime.
...
- **SC-026**: The core `microgpt.py` runs correctly under PyPy. Optional dependency layers (FastAPI, async SQLAlchemy, MLflow) gracefully detect CPython vs PyPy and report incompatibility rather than crashing.
...
- **SC-034**: Running `microgpt-workbench` CLI entry point (after `pip install`) starts the web server identically to `make run`. Running `microgpt-train --help` prints usage without errors.
...
- The core `microgpt.py` remains a standalone script runnable without the web server. The web server imports and orchestrates it.
- The entire system is distributable as a pip package (`pyproject.toml` with `microgpt-workbench`). `make` targets delegate to Python entry points. Docker containerization for Linux deployment is anticipated but not required for v1.
