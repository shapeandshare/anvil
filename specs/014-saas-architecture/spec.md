...
- **FR-057c**: The `get_mlflow_browser_uri(request)` function (in `anvil/config.py`) MUST produce CloudFront-aware URLs in SaaS mode. When `ANVIL_MODE=saas`, it MUST return `{request.base_url}v1/mlflow-proxy` using the CloudFront origin from the request's `Host` header and the `X-Forwarded-Proto` header for scheme (`https`). In local mode, the existing behavior (returning the direct MLflow subprocess URL on port 5001) MUST remain unchanged. This function is consumed by the experiments page, models page, and operations page to generate links to MLflow. A corresponding override service in `anvil/_saas/` MUST supply the SaaS-mode implementation. (FR-057c)
