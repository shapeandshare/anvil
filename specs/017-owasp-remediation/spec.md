...
**Feature Branch**: `feature/017-owasp-remediation`  
**Created**: 2026-06-21  
**Status**: Draft  
**Input**: OWASP Top 10 security review and finding tracker (36 findings across A01-A10 categories)
...
- **FR-031 (HIGH — auth migration safety)**: Enabling auth MUST be accompanied by coordinated updates so that dependent consumers do not break simultaneously: (a) the test HTTP client (`tests/conftest.py`) injects a test API key; (b) the Docker/compose healthcheck targets an exempt endpoint or is given credentials; (c) browser SSE relies on the cookie path (per FR-025); (d) the `/v1/health` exemption is confirmed to cover the healthcheck.
