...
| C-3 `/v1/` API/page collision + needless versioning | CRITICAL | Spun out → spec `018-header-api-versioning` + ADR-035 (greenfield, no back-compat) |
| C-4 API key written to logs | CRITICAL | FR-026; data-model; quickstart; T001/T004 |
| C-5 MLflow restriction breaking + insufficient | CRITICAL | FR-004 → reverse proxy; ADR-034; SaaS spec 014 unified; T009 |
...
- C-3 (URL de-versioning) is intentionally a SEPARATE feature (spec 018 + ADR-035); spec 017's auth contract works before and after it via an explicit page-route registry.
- New ADRs: **ADR-034** (MLflow reverse proxy), **ADR-035** (header-based versioning). SaaS spec 014 FR-057/FR-057c cross-referenced for the unified proxy.
- Status: revised post-review. Recommend re-running `/speckit.analyze` to re-validate FR↔task coverage before `/speckit.implement`.
