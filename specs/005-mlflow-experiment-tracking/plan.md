...
**Constraints**: Core engine (`microgpt/core/`) MUST remain stdlib-only — all MLflow/psutil/pynvml code lives in services/api, never in `core/` (the torch backend already lives in `core/torch_engine.py` and is import-guarded). Async throughout except core; imports at top of file; classes for logic; no circular imports; implicit namespace.
